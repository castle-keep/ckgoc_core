...
class CKBadge extends StatelessWidget {
  ...
  const CKBadge.success({required this.label, super.key})
    : variant = BadgeVariant.success;

  /// Warning badge variant (caution, pending).
  const CKBadge.warning({required this.label, super.key})
    : variant = BadgeVariant.warning;

  /// Error badge variant (critical, failed).
  const CKBadge.error({required this.label, super.key})
    : variant = BadgeVariant.error;

  /// Info badge variant (informational).
  const CKBadge.info({required this.label, super.key})
    : variant = BadgeVariant.info;

  /// Draft badge variant (work in progress).
  const CKBadge.draft({required this.label, super.key})
    : variant = BadgeVariant.draft;

  /// Live badge variant (currently active).
  const CKBadge.live({required this.label, super.key})
    : variant = BadgeVariant.live;

  /// New badge variant (new feature/item).
  const CKBadge.newBadge({required this.label, super.key})
    : variant = BadgeVariant.newBadge;

  /// Beta badge variant (beta program).
  const CKBadge.beta({required this.label, super.key})
    : variant = BadgeVariant.beta;

  /// Pro badge variant (premium feature).
  const CKBadge.pro({required this.label, super.key})
    : variant = BadgeVariant.pro;

  /// Outline badge variant (bordered, minimal).
  const CKBadge.outline({required this.label, super.key})
    : variant = BadgeVariant.outline;

  /// Outline success badge variant.
  const CKBadge.outlineSuccess({required this.label, super.key})
    : variant = BadgeVariant.outlineSuccess;

  /// Outline error badge variant.
  const CKBadge.outlineError({required this.label, super.key})
    : variant = BadgeVariant.outlineError;

  /// Online status badge.
  const CKBadge.online({required this.label, super.key})
    : variant = BadgeVariant.online;

  /// Away status badge.
  const CKBadge.away({required this.label, super.key})
    : variant = BadgeVariant.away;

  /// Busy status badge.
  const CKBadge.busy({required this.label, super.key})
    : variant = BadgeVariant.busy;

  /// Offline status badge.
  const CKBadge.offline({required this.label, super.key})
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(label, style: theme.typography.labelSm.copyWith(color: fg)),
    );
  }
