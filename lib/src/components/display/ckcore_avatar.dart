...
double _dp(AvatarSize s, ckcoreSpacing spacing) => switch (s) {
...
double _fontSize(AvatarSize s, ckcoreSpacing spacing) =>
    (_dp(s, spacing) * 0.375).roundToDouble();

// Dot is 25% of diameter, clamped sm–s12.
double _dotSize(AvatarSize s, ckcoreSpacing spacing) =>
...
class CKAvatar extends StatelessWidget {
  ...
  (Color bg, Color fg) _resolveColors(ckcoreColors colors) {
  ...
  Color _statusColor(ckcoreColors colors) => switch (status!) {
...
/// Compact avatar group that shows overlapping `CKAvatar` instances.
class CKAvatarGroup extends StatelessWidget {
  ...
  Widget _ringed(CKAvatar avatar, double dp, double ring, ckcoreColors colors) {
    return Container(
      width: dp + ring * 2,
      height: dp + ring * 2,
...
/// Deprecated: Use [CKAvatar] instead.
@Deprecated('Use CKAvatar instead')
typedef ckcoreavatar = CKAvatar;
